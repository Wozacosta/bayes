 ...
 */
Naivebayes.prototype.initializeCategory = categoryName => {
 ...
 * @param  {String} class
 */
Naivebayes.prototype.learn = (text, category) => {
 ...
 * @param  {String} class
 */
Naivebayes.prototype.unlearn = (text, category) => {
  //update our count of how many documents mapped to this category
  this.docCount[category]--;
 ...
 */
Naivebayes.prototype.categorizeObj = text => {
 ...
 */
Naivebayes.prototype.categorize = text => {
  let maxProbability = -Infinity,
  ...
  Object.keys(this.categories).forEach(category => {
    ...
    if (logProbability == Number.NEGATIVE_INFINITY) {
      console.error(
        `category ${category} had -Infinity odds, didn't add it to the categoryProbas array`
      );
    } else {
  ...
  // from log-likelihood probas get more human readable values (0 = less probable -100 most probable)
  let totalNum = categoryProbas.length;
  let bigDelta = maxProbability - minProbability;

  categoryProbas.forEach((value, index) => {
 ...
 */
Naivebayes.prototype.tokenProbability = (token, category) => {
 ...
 * @return {Object}
 */
Naivebayes.prototype.frequencyTable = tokens => {
